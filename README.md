# [:es:](README_ES.md)  [:fr:](README_FR.md) [:de:](README_DE.md)
>[!CAUTION]
> This project is in process of developing, please be carefull when you will use it.

>## Automatization
<!-- Image Cover -->
<p align="center">
 <img src="icon_rename.png" width="300">
</p>
<!-- HeadLine -->

# Rename files

This is a standalone executable designed to batch rename any type of file into a sequential numerical order, regardless of their original format. It is ideal for organizing comic pages, manga, or image collections.

## Functions
| | |
|:---:| :---: |
|  <img src="gifs/rename_clips_1.gif" width="450"> | <img src="gifs/rename_clips_4.gif" width="450"> |
| Write your name for all pages |  Click in "change names" to select the folder |
| <img src="gifs/rename_clips_2.gif" width="450"> |  <img src="gifs/rename_clips_3.gif" width="450"> |
| Select the folder where are the files that you want to rename | Your files are renamed |
|  <img src="gifs/rename_clips_5.gif" width="450"> | <img src="gifs/rename_clips_6.gif" width="450"> |
| **Odds:** The files are renamed only in odd numbers |  **Evens:** The files are renamed only in even numbers |

### How to Use
1. Go to the directory dist and click on the file .`exe`.
2. Run the application.
3. Select your preferred sorting mode (Sequential, Even, or Odd).
4. Select the folder where you want to change the name of your files.
5. Done! Your files will be renamed instantly.

## How to install plugin
You can download this plugin with the folowing command:
```
git clone https://github.com/jartibledev/order-comic-pages.git 
```
or download the zip.


## Issues
>[!WARNING]
> Don´t use this plugin in directories as Desktop and Users. Only in folders that you are created for your projects.

## About the author 
If you want know more about me, take a look to my [profile](https://github.com/jartibledev).



